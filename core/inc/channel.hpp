...
public:
    ...
    virtual void extractAndProcessFrame(size_t trigger_offset, size_t frame_width) = 0;
...
public:
    ...
    virtual void process(const std::vector<IChannel*>& sources, std::vector<Trace>& traces) = 0;
};
...
public:
    ...
    void addSource(IChannel* source) {
    ...
    float getNormalizedSample(size_t /*index_offset*/) const override { return 0.0f; } 
    ...
    void extractAndProcessFrame(size_t /*trigger_offset*/, size_t /*frame_width*/) override {
        m_traces.clear();
...
/// Encapsulates the acquisition and processing pipeline for a single hardware channel.
...
public:
  ...
  void extractAndProcessFrame(size_t trigger_offset, size_t frame_width) override {
    ...
    if (start + frame_width <= capacity) {
      std::copy(m_buffer.getRawData() + start,
                m_buffer.getRawData() + start + frame_width, m_raw_frame.begin());
    } else {
  ...
  void addProcessor(std::unique_ptr<IProcessor<HardwareT>> proc) { m_processors.push_back(std::move(proc)); }
  ...
  float getNormalizedSample(size_t index_offset) const override {
      if (index_offset >= m_buffer.getUnreadCount()) return 0.0f;
  ...
  void pushRawBytes(const uint8_t* data, size_t size) override {
      ...
      if constexpr (std::is_same_v<HardwareT, uint8_t>) {
          ...
          for (size_t i = 0; i < size; ++i) {
              m_buffer.push(static_cast<HardwareT>(data[i]));
          }
      }
  }
