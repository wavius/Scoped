...
/// The central hub of the application. Owns all channels, hardware links, and the global trigger engine.
class Oscilloscope {
private:
...
public:
    ...
    const std::vector<std::shared_ptr<IChannel>>& getChannels() const { return m_channels; }
    ...
    void update() {
        ...
        size_t src_idx = m_trigger_source_idx < m_channels.size() ? m_trigger_source_idx : 0;
        auto& source_channel = m_channels[src_idx];

        size_t trigger_offset = 0;
        if (m_trigger && m_trigger->processStream(source_channel.get(), trigger_offset)) {
